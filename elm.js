...
var $author$project$Main$SetLocal = F2(
	function (a, b) {
		return {$: 'SetLocal', a: a, b: b};
	});
var $author$project$Main$TriggerDownloadPort = {$: 'TriggerDownloadPort'};
...
var $author$project$Polygon$normalizeAngle = function (x) {
	return x - (360 * $elm$core$Basics$floor(x / 360));
};
var $author$project$Polygon$anglesEqual = F2(
	function (a, b) {
		var diff = $author$project$Polygon$normalizeAngle(a - b);
		return (_Utils_cmp(diff, $author$project$Util$epsilon) < 0) || (_Utils_cmp(diff, 360 - $author$project$Util$epsilon) > 0);
	});
var $author$project$Polygon$equals = F2(
	function (p1, p2) {
		return _Utils_eq(p1.lengths, p2.lengths) && (_Utils_eq(p1.angles, p2.angles) && A2($author$project$Polygon$anglesEqual, p1.rotation, p2.rotation));
	});
...
var $author$project$Rules$applies = F2(
	function (rule, p) {
		return (rule.subdivide ? true : _Utils_eq(rule.anchor.scale, p.scale)) && (rule.rotatable ? A2($author$project$Rules$eq2, rule.anchor, p) : A2($author$project$Rules$eq, rule.anchor, p));
	});
...
var $author$project$Rules$sc = F2(
	function (scalar, pc) {
		var p = pc.poly;
		return _Utils_update(
			pc,
			{
				centre: A2(
					...
					A2($author$project$Util$mul, scalar, p.origin),
					A2(
						$author$project$Util$mul,
						scalar,
						A2($author$project$Util$sub, pc.centre, p.origin))),
				dist: pc.dist * scalar,
				poly: _Utils_update(
					p,
					{
						origin: A2($author$project$Util$mul, scalar, p.origin)
					}),
				scale: pc.scale * scalar
			});
	});
...
var $author$project$Rules$modify = F3(
	function (tess, p, rest) {
		var subs = A2(
						...
						return A2(
							...
							A2(
								...
								A3(
									...
									rule.anchor.poly.origin,
									p.poly.rotation - rule.anchor.poly.rotation,
									A2($author$project$Rules$sc, p.scale / rule.anchor.scale, p2))));
					},
					rule.additions);
			},
			A2(
				...
				A2(
					$elm$core$List$filter,
					function (rule) {
						return _Utils_cmp(rule.fragment, p.scale) < 1;
					},
					A2(
						$elm$core$List$filter,
						function (rule) {
							return rule.subdivide;
						},
						tess.rules))));
		var adds = A2(
			$elm$core$List$concatMap,
		...
		return _Utils_update(
			tess,
			{
				closed: $elm$core$List$isEmpty(subs) ? A2($elm$core$List$cons, p, tess.closed) : tess.closed,
				open: _Utils_ap(
					subs,
					_Utils_ap(rest, adds))
			});
...
var $author$project$Rules$keep0 = F3(
	function (val, x, y) {
		return (!val) ? x : y;
	});
var $author$project$Rules$placeStart = F3(
	function (tess, w, h) {
		var centre = function () {
			var _v0 = $elm$core$List$head(tess.rules);
			if (_v0.$ === 'Nothing') {
				return {x: 0, y: 0};
			} else {
				var rule = _v0.a;
				return rule.anchor.centre;
			}
		}();
		var distance = {
			x: A3($author$project$Rules$keep0, tess.start.x, 0, ((tess.start.x * w) / tess.size) - centre.x),
			y: A3($author$project$Rules$keep0, tess.start.y, 0, ((tess.start.y * h) / tess.size) - centre.y)
		};
		return _Utils_update(
			tess,
			{
				closed: A2(
					$elm$core$List$map,
					$author$project$Rules$tr(distance),
					tess.closed),
				open: A2(
					$elm$core$List$map,
					$author$project$Rules$tr(distance),
...
var $author$project$Polygon$polygonAnimatedSvg = F7(
	function (poly, size, origin, color, w, index, total) {
		...
		var delay = A2($author$project$Polygon$calcDelay, index, total);
		return A2(
			$elm$svg$Svg$polygon,
			_List_fromArray(
				[
...
var $author$project$Rules$rescale = function (pc) {
	var p = pc.poly;
	return _Utils_update(
		pc,
		{
...
var $author$project$Rules$renderAnimatedTess = function (_v0) {
	...
	return A2(
		...
		A2($elm$core$List$map, $author$project$Rules$rescale, closed));
};
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					$elm$svg$Svg$Attributes$class('tess-svg')
				]),
			A2(
...
var $author$project$Rules$flr = {
	centre: {
		x: 1 + $elm$core$Basics$cos(
			$elm$core$Basics$degrees(60)),
		y: ($elm$core$Basics$sin(
			$elm$core$Basics$degrees(60)) * (1 + (2 * $elm$core$Basics$cos(
			$elm$core$Basics$degrees(60))))) / 2
	},
	col: $author$project$Util$Primary,
	dist: 0.5,
	poly: $author$project$Shapes$floret,
	scale: 1
};
var $author$project$Util$mag = function (_v0) {
	var x = _v0.x;
	var y = _v0.y;
...
var $author$project$Rules$ois = {
	centre: A2(
		$author$project$Util$mul,
		2 / 3,
		{
			x: 0.25,
			y: $elm$core$Basics$cos(
				$elm$core$Basics$degrees(30)) / 2
		}),
	col: $author$project$Util$Primary,
	dist: $author$project$Util$mag(
		{
			x: 0.25,
			y: $elm$core$Basics$cos(
				$elm$core$Basics$degrees(30)) / 2
...
var $author$project$Rules$pt = F2(
	function (alfa, pc) {
		var prevP = A2(
			...
			$author$project$Rules$rescale(pc).poly);
		var nextP = A2(
			$author$project$Polygon$getPoint,
...
var $author$project$Rules$rho = {
	...
	col: $author$project$Util$Primary,
	dist: ($elm$core$Basics$cos(
		$elm$core$Basics$degrees(30)) * $elm$core$Basics$cos(
		$elm$core$Basics$degrees(30))) / 2,
...
var $author$project$RuleBased$Fractal$floretFractalTess = function () {
	var petal1 = _Utils_update(
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					A2($author$project$Rules$rto, -60, $author$project$Rules$flr)
				]),
			anchor: $author$project$Rules$flr,
			bounds: _Utils_Tuple2(
				{x: -0.75, y: 0},
				{x: 3, y: 2.5})
		});
	var next3 = _Utils_update(
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
	...
	var fractal = _Utils_update(
		$author$project$Rules$r,
		{
			...
			anchor: A2($author$project$Rules$rto, -60, $author$project$Rules$flr),
			bounds: _Utils_Tuple2(
				{x: -1, y: 0},
				{x: 2, y: 2.5}),
			rotatable: true,
...
var $author$project$RuleBased$Isogonal$hexaStarTess = function () {
			...
			additions: _List_fromArray(
				[
					...
					A2(
						$author$project$Rules$tr,
						{x: -1, y: 0},
						A2($author$project$Rules$sc, 2, $author$project$Rules$hexv))),
					A2(
					$author$project$Rules$tr,
	...
	var hexRule1 = _Utils_update(
		$author$project$Rules$r,
		{
			...
			anchor: A2($author$project$Rules$sc, 2, $author$project$Rules$hexv),
			bounds: _Utils_Tuple2(
				{x: -1.2, y: -0.5},
				{x: 5, y: 5})
		});
...
var $author$project$RuleBased$Isogonal$pythagoreanTess = function () {
	...
	var squareRule1 = _Utils_update(
		$author$project$Rules$r,
		{
			...
			anchor: A2($author$project$Rules$sc, 3, $author$project$Rules$squ),
			bounds: _Utils_Tuple2(
				{x: -1, y: -1},
				{x: 5, y: 5}),
			rotatable: false
...
var $author$project$RuleBased$Isogonal$trithagoreanTess = function () {
	...
	var triRule1 = _Utils_update(
		$author$project$Rules$r,
		{
			...
			anchor: A2($author$project$Rules$sc, 3, $author$project$Rules$eqi),
			bounds: _Utils_Tuple2(
				{x: -0.3, y: -1.2},
				{x: 4, y: 4}),
			rotatable: false
...
var $author$project$Rules$car = {
	centre: {
		x: $elm$core$Basics$sqrt(2) * $elm$core$Basics$cos(
			$elm$core$Basics$degrees(75)),
		y: ($elm$core$Basics$cos(
			$elm$core$Basics$degrees(30)) + $elm$core$Basics$sin(
			$elm$core$Basics$degrees(30))) / 2
	},
	col: $author$project$Util$Primary,
	dist: 0.5,
...
var $author$project$RuleBased$Laves$cairoPentagonalTess = function () {
	var upR = _Utils_update(
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					...
					A2(
						$author$project$Rules$pt,
						3,
						A2($author$project$Rules$rto, 180, $author$project$Rules$car)),
			...
			anchor: A2(
				...
				180,
				_Utils_update(
					$author$project$Rules$car,
					{col: $author$project$Util$Ternary})),
			bounds: _Utils_Tuple2(
				{x: -1, y: -2.5},
				{x: 2, y: 2.5})
		});
	var rightR = _Utils_update(
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					...
					A2(
						...
						1,
						A2($author$project$Rules$rto, 90, $author$project$Rules$car)),
					A2($author$project$Rules$rto, 270, $author$project$Rules$car))
				]),
			anchor: A2(
				$author$project$Rules$rto,
				90,
		...
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					...
					A2(
					$author$project$Rules$tr,
					A2($author$project$Rules$pt, 1, $author$project$Rules$car),
					A2(
						$author$project$Rules$tr,
						A2(
							$author$project$Rules$pt,
					...
					A2(
						$author$project$Util$sub,
						A2($author$project$Rules$pt, 2, $author$project$Rules$car),
						A2($author$project$Rules$pt, 4, $author$project$Rules$car)),
					A2(
						$author$project$Rules$tr,
						A2(
							$author$project$Rules$pt,
	...
	var rhomb1 = _Utils_update(
		$author$project$Rules$r,
		{
			...
			anchor: A2($author$project$Rules$rto, 90, $author$project$Rules$car),
			bounds: _Utils_Tuple2(
				{x: 0, y: -1.8},
				{x: 3.5, y: 3})
		});
	var leftR = _Utils_update(
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
...
var $author$project$Rules$kit = function () {
	var c = A2(
		$author$project$Util$add,
		A2(
			$author$project$Util$mul,
			...
			A2(
				$author$project$Util$sub,
				A2($author$project$Polygon$getPoint, 3, $author$project$Shapes$kite),
...
var $author$project$RuleBased$Laves$deltodailTriHexagonalTess = function () {
	...
	var subRule1 = _Utils_update(
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					_Utils_update(
					$author$project$Rules$kit,
					{col: $author$project$Util$Secondary}),
					A2(
					$author$project$Rules$tr,
					A2($author$project$Rules$pt, 1, bigT),
					A2(
						$author$project$Rules$rto,
						-120,
						_Utils_update(
					...
					A2(
					$author$project$Rules$tr,
					A2($author$project$Rules$pt, 2, bigT),
					A2($author$project$Rules$rto, 120, $author$project$Rules$kit))
				]),
			anchor: bigT,
			bounds: _Utils_Tuple2(
				{x: 0, y: 0},
	...
	var subRule2 = _Utils_update(
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					A2($author$project$Rules$rto, -60, $author$project$Rules$kit),
					A2(
					$author$project$Rules$tr,
					A2(
						$author$project$Rules$pt,
						1,
						A2($author$project$Rules$rto, -60, bigT)),
					A2(
						$author$project$Rules$rto,
						-180,
						_Utils_update(
				...
				[
					...
					A2(
						...
						A2(
							$author$project$Rules$tr,
							A2($author$project$Rules$pt, 1, $author$project$Rules$flr),
							A2($author$project$Rules$rto, 180, $author$project$Rules$flr))))
				]),
			anchor: A2(
				$author$project$Rules$rto,
				-120,
...
var $author$project$RuleBased$Laves$prismaticPentagonalTess = function () {
		...
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					...
					$author$project$Rules$tr,
					{x: 1, y: 0},
					A2($author$project$Rules$rto, 180, $author$project$Rules$pri))
				]),
			anchor: _Utils_update(
	...
	var up1 = _Utils_update(
		$author$project$Rules$r,
		{
			...
			anchor: $author$project$Rules$pri,
			bounds: _Utils_Tuple2(
				{x: 0, y: -1.5},
				{x: 1, y: 3})
		});
	var prism3 = _Utils_update(
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
		...
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					...
					A2(
						$author$project$Rules$tr,
						A2($author$project$Rules$pt, 3, $author$project$Rules$pri),
						A2($author$project$Rules$rto, 180, $author$project$Rules$pri)))
				]),
			anchor: _Utils_update(
				$author$project$Rules$pri,
		...
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					...
					A2($author$project$Rules$rto, -135, $author$project$Rules$iso)),
					A2(
					$author$project$Rules$tr,
					A2(
						$author$project$Rules$sc,
			...
			bounds: _Utils_Tuple2(
				{x: 0, y: 0},
				{
					x: $elm$core$Basics$sqrt(2),
					y: $elm$core$Basics$sqrt(2)
				}),
			rotatable: false,
			subdivide: true
		});
	var sqTile = _Utils_update(
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
		...
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					A2(
					$author$project$Rules$tr,
					A2($author$project$Rules$rto, -60, bigT).centre,
				...
				[
					...
					$author$project$Util$neg(
						A2(
							$author$project$Rules$pt,
							1,
							A2($author$project$Rules$rto, 30, $author$project$Rules$squ))),
					A2(
						$author$project$Rules$rto,
						30,
						_Utils_update(
...
var $author$project$RuleBased$Semiregular$snubSquareTess = function () {
	...
	var triRight = _Utils_update(
		$author$project$Rules$r,
		{
			...
			anchor: A2($author$project$Rules$rto, -30, $author$project$Rules$eqi),
			bounds: _Utils_Tuple2(
				{x: 0, y: -1},
				{x: 2, y: 3})
		});
...
var $author$project$RuleBased$Semiregular$snubTriHexagonalTess = function () {
	...
	var diag = $author$project$Util$neg(
		A2($author$project$Rules$pt, 5, $author$project$Rules$hexv));
	var hexRule2 = _Utils_update(
		$author$project$Rules$r,
		{
			additions: _List_fromArray(
				[
					...
					$author$project$Util$neg(
						A2($author$project$Rules$pt, 4, $author$project$Rules$hexv)),
					A2($author$project$Rules$tr, diag, $author$project$Rules$hexv)),
					A2(
					$author$project$Rules$tr,
					{x: 2, y: 0},
					A2($author$project$Rules$tr, diag, $author$project$Rules$hexv)),
			...
			case 'SelectTheme':
				...
				return _Utils_Tuple2(
					_Utils_update(
						...
						{selectedTheme: theme}),
					$author$project$Main$run(
						A2($author$project$Main$SetLocal, 'tess-theme', theme)));
			case 'PickStroke':
				var color = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
			...
			case 'PickPrimary':
				...
				return _Utils_Tuple2(
					_Utils_update(
						...
						{customPrimary: color}),
					$author$project$Main$run(
						A2($author$project$Main$SetLocal, 'tess-primary', color)));
			case 'PickSecondary':
				var color = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
					...
					$author$project$Main$run(
						A2($author$project$Main$SetLocal, 'tess-secondary', color)));
			case 'PickTernary':
				var color = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						...
						{customTernary: color}),
					$author$project$Main$run(
						A2($author$project$Main$SetLocal, 'tess-ternary', color)));
			case 'PickQuart':
				var color = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						...
						{customQuart: color}),
					$author$project$Main$run(
						A2($author$project$Main$SetLocal, 'tess-quart', color)));
			case 'DownloadSvg':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							downloadData: A4($author$project$Main$currentSvg, model, false, 1920, 1080)
						}),
					$author$project$Main$run($author$project$Main$TriggerDownloadPort));
			case 'TriggerDownloadPort':
				return _Utils_Tuple2(
					model,
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A2($elm$html$Html$map, $elm$core$Basics$never, model.downloadData)
					]))
			]));
};
...
var $author$project$Rules$renderRule = function (_v0) {
	...
	var addSvgs = A2(
		...
		A2($elm$core$List$map, $author$project$Rules$rescale, additions));
	var _v1 = bounds;
	var tl1 = _v1.a;
	var br1 = _v1.b;
	var br = A2(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$id('tess-text')
					]),
				_List_fromArray(
					[
	...
	A2(
		...
		function (theme) {
			return A2(
				...
				A2($elm$json$Json$Decode$field, 'tessellation', $elm$json$Json$Decode$string));
		},
